...
"""
Version adaptée au CSV masculin player_data_atp.csv.

Attentes de colonnes (exemples) :
full_name,player_id,represented_country,height_inches,height_cm,plays,backhand,
birth_date,birthplace,first_appearance,last_appearance,highest_ranking,prize_money

Ce script reprend la logique de migration_map_from.py (WTA) en adaptant :
 - parsing des dates (ISO + formats courts)
 - parsing des tailles (ex: "6' 3\"\"", "1.91m")
 - player_id gardé comme string (ex: 'S0AG')
 - lecture de highest_ranking comme best_rank
 - utilise geocode_utils_atp.geocode_place / reverse_to_iso3
"""
import os
import time
import json
...
# utilitaires de geocodage spécialisés pour l'ATP (fichier fourni séparément)
...
def save_cache_safe(cache: dict, path: str):
    """Wrapper sur save_cache."""
    save_cache(cache, path)


# ── Helpers pour parsing de dates / tailles / ranks ───────────────────────────
...
def parse_birth_date(raw: str):
    """Retourne date ISO 'YYYY-MM-DD' ou None. Accepte plusieurs formats."""
    ...
    for patt in _DATE_PATTERNS:
        try:
            ...
            continue
    # dernier recours : retirer tous les caractères non alphanumériques sauf espaces
...
def parse_height_to_meters(height_cm_field, height_inches_field=None):
    """
    Prend un champ height_cm (ex: '1.91m' ou '1.91') ou height_inches (ex: '6' 3""')
    et retourne float (mètres) ou None.
    """
    # try height_cm first
    h = height_cm_field
    if isinstance(h, str):
        ...
        try:
            val = float(s)
            # if value looks like centimeters (e.g. 191), convert
            if val > 3:  # improbable to have > 3 meters -> cm likely
                return val / 100.0
            return val
        except Exception:
            pass

    # fallback to parse inches string
    h2 = height_inches_field
    if isinstance(h2, str) and h2.strip():
        s = h2.strip()
        # examples: 6' 3"", 6'3", 6' 3"
        ...
        if m:
            try:
                ...
                pass
        # sometimes format is like 6' or 6 ft
...
def parse_best_rank(row):
    """Gère highest_ranking ou best_rank, retourne float('inf') si absent."""
    for k in ('highest_ranking', 'best_rank', 'best_rank_value'):
        if k in row and pd.notna(row[k]) and str(row[k]).strip() != '':
            try:
                v = str(row[k]).strip()
                return float(v)
            except Exception:
                # parfois des valeurs non-numeric (e.g. '-'), essayer extraire chiffre
...
def load_and_normalize_from_atp(ioc_to_iso3: dict, csv_path: str) -> pd.DataFrame:
    """
    Charge le CSV ATP et normalise les colonnes attendues.
    - Convertit represented_country via ioc_to_iso3 si nécessaire.
    - Garde uniquement lignes avec birth_date et birthplace non null.
    - Ajoute/normalize colonnes: represented_country (ISO3), birth_date (ISO), best_rank (float)
    """
    df = pd.read_csv(csv_path, dtype=str)  # lire tout en str pour robustesse

    # trim des colonnes existantes
    df = df.applymap(lambda x: x.strip() if isinstance(x, str) else x)

    # normaliser represented_country via IOC map s'il y a lieu
    ...
    def map_country(c):
        ...
        if s in ioc_map:
            return ioc_map[s]
        # si déjà iso3 valide -> garder
        try:
            if pycountry.countries.get(alpha_3=s):
                return s
        except Exception:
            pass
        # si alpha_2 -> convertir
        try:
            if pycountry.countries.get(alpha_2=s):
                return pycountry.countries.get(alpha_2=s).alpha_3
        except Exception:
            pass
        return s
    ...
    # filter rows: require birth_date_iso and birthplace
    df = df[df['birth_date_iso'].notna() & df.get('birthplace', pd.Series()).notna()]

    # Normalize best_rank from highest_ranking
    df['best_rank'] = df.apply(parse_best_rank, axis=1)

    # normalize heights: add height_m column
    df['height_m'] = df.apply(
        lambda r: parse_height_to_meters(r.get('height_cm', None), r.get('height_inches', None)),
        axis=1
    )

    # standardize plays lowercase (Right-Handed, Left-Handed)
    df['plays_norm'] = df.get('plays', pd.Series()).fillna('').astype(str).str.lower()

    # keep only rows with valid represented_country (ISO3) if possible
    valid_iso3 = {c.alpha_3 for c in pycountry.countries}
    df = df[df['represented_country'].isin(valid_iso3)]

    # ensure full_name exists
    df['full_name'] = df.get('full_name', df.get('name', pd.Series()))
...
def build_points_and_migrations_from_atp(cache_file: str, geolocator, df: pd.DataFrame, cache: dict):
    """
    ...
    Signature acceptant 'geolocator' pour être compatible avec main_maps_atp.
    Si geolocator est None, on en crée un via get_geolocator().
    """
    # assure cache loaded
    if not cache:
        cache = load_cache_safe(cache_file)

    # si l'appelant fournit un geolocator, on l'utilise, sinon on en crée un
    ...
    for _, row in df.iterrows():
        ...
        if from_iso and from_iso != to_iso:
            ...
            dest_coords = None
            if dest_name:
                # prefer geocode via geolocator if available (use same geocode wrapper if present)
                # the geocode_place function writes/reads cache; use it for consistency
                dest_coords = geocode_place(dest_name, cache, cache_file,
                                            user_agent="migration-atp", delay=1.0, timeout=10)
            if not dest_coords:
                # fallback: try a direct geolocator.geocode call (non-cached) as a last resort
                try:
                    loc = geolocator.geocode(dest_name, timeout=10) if dest_name else None
                    if loc:
                        dest_coords = (float(loc.latitude), float(loc.longitude))
                except Exception:
                    dest_coords = None

            if not dest_coords:
                # cannot determine destination coords -> skip migration record
                continue
...
def build_and_save_map_migration_from_atp(all_pts, migrations, out_html: str):
    ...
    # choropleth setup
    outbound_counts = Counter(rec['from_iso'] for rec in migrations)
    max_count = max(outbound_counts.values(), default=1)
    colormap = linear.YlOrRd_09.scale(0, max_count)
    colormap.caption = "Number of emigrant players"
    colormap.add_to(map_obj)
    count2color = {i: colormap(i) for i in range(0, max_count + 1)}

    # inject data variables for client-side script
    ...
    folium.GeoJson(
        ...
        style_function=lambda feature: {
            'fillColor':   colormap(outbound_counts.get(feature['id'], 0)),
            'color':       '#999',
            'weight':      1,
            'fillOpacity': 0.7
        },
      ...
      function buildAllLinesOnce() {
        ...
        migrations.forEach((m, idx) => {
          try {
            ...
            migrationLines[m.from_iso] = migrationLines[m.from_iso] || [];
            migrationLines[m.from_iso].push(line);
