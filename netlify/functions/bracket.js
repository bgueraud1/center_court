...
const NEXT_INSCRIPTIONS_TABLE = process.env.NEXT_INSCRIPTIONS_TABLE || process.env.INSCRIPTIONS_TABLE || 'next_inscriptions';
...
async function supabaseInsert(table, payload) {
  return await supabaseRequest(table, { method: 'POST', query: { select: '*' }, payload: [payload] });
}
...
async function resolveUser(ctx) {
  ...
  if (base.user_id) {
    ...
    if (Array.isArray(rows) && rows.length) {
      ...
      return {
        user_id: u.id,
        user_name: u.pseudo || base.user_name,
        user_tour: u.tour || base.user_tour,
        user_country: u.country || base.user_country,
        user_league: u.league || base.user_league,
    ...
    return { ...base, source: 'headers.user_id' };
  }
  ...
  if (base.user_name) {
    ...
    return { ...base, user_id: null, source: 'headers.user_name' };
  }
...
function chooseCurrentBracketRow(rows) {
  ...
  return rows.find(r => r.current_tournament_bracket_id || r.user_current_tournament_bracket_proposition || r.current_tournament_bracket) || rows[0] || null;
}
...
function parseBracketJson(value) {
...
function normalizeCurrentRow(row) {
  if (!row) return null;
  const currentTournamentBracket = parseBracketJson(row.current_tournament_bracket);
  const proposition = parseBracketJson(row.user_current_tournament_bracket_proposition);
  ...
  return {
    ...
    current_tournament_bracket: currentTournamentBracket,
    user_current_tournament_bracket_proposition: row.user_current_tournament_bracket_proposition ?? null,
    user_current_tournament_bracket_proposition_json: proposition,
...
function normalizeNextRow(row, templateJson) {
  if (!row) return null;
  const proposition = parseBracketJson(row.user_proposition_next_week);
  ...
  return {
    ...
    user_proposition_next_week_json: proposition,
    template_json: templateJson,
    template_mode: templateJson ? (isAllMode(templateJson) ? 'all' : 'first_round') : 'missing',
    editable: !row.user_proposition_next_week && !!templateJson,
...
function validateProposalObject(input) {
  const parsed = parseBracketJson(input);
...
exports.handler = async (event) => {
  try {
    ...
    if (method === 'POST' && String(body.action || '').toLowerCase() === 'save_next_week_proposition') {
      ...
      const proposalObject = parseBracketJson(proposalSource);
