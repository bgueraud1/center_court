...
def scrape_one_date(date_str: str, save_dir: str) -> None:
    """Point d'entrée pour un workflow qui passe une seule date."""
    current_date = parse_iso_date(date_str)
    scrape_data([current_date], save_dir)

def parse_args():
    parser = argparse.ArgumentParser(description="WTA scraper for a single date.")
    parser.add_argument(
        "--date",
        required=True,
        help="Date to scrape in YYYY-MM-DD format."
    )
    parser.add_argument(
        "--save-dir",
        default="wta_rankings",
...
if __name__ == "__main__":
    args = parse_args()
    scrape_one_date(args.date, args.save_dir)